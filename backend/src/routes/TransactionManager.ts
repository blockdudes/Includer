...
const contractAddress =
  "CAK2YJSZ35QYPHWVHPRTKPPDYD5CT5YPXCC55HHUA6NQEMZMTQX4TYWY";
const tokenContractAddress =
  "CDQ56Q2MGOI53XVQMARVHFWI6FVM7FG7QHAT22Q2Z7L7W5WU6IYXED4V";
...
async function waitForTransactionCompletion(
  ...
  while (true) {
    transactionStatus = (await server.getTransaction(
      txHash
...
async function setAllowance(
  ...
  const ownerScAddress = xdr.ScVal.scvAddress(
    new Address(ownerAddress).toScAddress()
  );
  const spenderScAddress = xdr.ScVal.scvAddress(
    new Address(spenderAddress).toScAddress()
  );
...
async function transferToken(
  ...
  const fromScAddress = xdr.ScVal.scvAddress(
    new Address(fromAddress).toScAddress()
  );
  const toScAddress = xdr.ScVal.scvAddress(
    new Address(toAddress).toScAddress()
  );
  const amountScVal = xdr.ScVal.scvI128(
    new xdr.Int128Parts({
      lo: xdr.Uint64.fromString(amount.toString()),
...
function convertTransactionResponse(response: any): TransactionResponse {
  ...
  if (response.status === "SUCCESS" && response.resultMetaXdr) {
    return {
      ...
      hash: response.hash,
      resultMetaXdr: response.resultMetaXdr.toXDR
        ? response.resultMetaXdr.toXDR("base64")
        : undefined, // Convert if possible
    };
  }
...
export async function mintToken(toAddress: string) {
  const tokenContractAddress =
    "CDQ56Q2MGOI53XVQMARVHFWI6FVM7FG7QHAT22Q2Z7L7W5WU6IYXED4V";
...
async function buildContractOperation(
  ...
  const userScAddress = xdr.ScVal.scvAddress(
    new Address(userAddress).toScAddress()
  );
  const amountScVal = xdr.ScVal.scvI128(
    new xdr.Int128Parts({
      lo: xdr.Uint64.fromString(amount.toString()),
  ...
  if (contractMethod === "deposit") {
    const token = xdr.ScVal.scvAddress(
      new Address(tokenContractAddress).toScAddress()
    );
...
async function handleTransaction(
  ...
  const operation = await buildContractOperation(
    contractMethod,
    publicKey,
    amount
  );

  const response = await buildAndSubmitTransaction(
    publicKey,
    operation,
    keypair
  );
  ...
  if (response.status !== "SUCCESS") {
    throw new Error(
      `Transaction failed: ${response.resultMetaXdr || "Unknown error"}`
    );
  }
...
async function getBalance(email: string) {
  ...
  const tx = new TransactionBuilder(source, {
    ...
    .addOperation(contract.call("get_user_balance", userScAddress))
    .setTimeout(30)
    .build();
...
async function transferFunds(
  ...
  try {
    ...
    res
      .status(500)
      .send({ message: "Transfer failed", error: (error as Error).message });
  }
}
...
transactionManager.post("/deposit", (req, res) =>
  executeTransaction(req, res, "deposit")
);
transactionManager.post("/repay", authenticateToken, (req, res) =>
  executeTransaction(req, res, "repay_token")
);
transactionManager.post("/withdraw", authenticateToken, (req, res) =>
  executeTransaction(req, res, "withdraw_token")
);
transactionManager.post("/borrow", authenticateToken, (req, res) =>
  executeTransaction(req, res, "borrow_token")
);
transactionManager.post("/setAllowance", authenticateToken, (req, res) =>
  executeTransaction(req, res, "setAllowance")
);
transactionManager.post("/transfer", authenticateToken, (req, res) =>
  transferFunds(req, res)
);

const jsonString = (data: any) =>
...
transactionManager.get("/getBalance/:email", authenticateToken, (req, res) => {
  ...
  getBalance(email)
    .then((result) =>
      res
        .status(200)
        .send({ message: "Balance fetched successfully", balance: result })
    )
    .catch((error) =>
      res
        .status(500)
        .send({ message: "Failed to fetch balance", error: error.message })
    );
});
